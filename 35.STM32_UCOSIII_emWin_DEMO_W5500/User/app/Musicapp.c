...
static uint8_t  musicfile_num;																	//存储扫描到的MP3+WAV音频文件数量 
						...
						{ 
								 ...
								 strstr(fn,".flac")||strstr(fn,".FLAC"))//判断是否mp3或wav文件
							{
								if (strlen(path)+strlen(fn)<FILE_NAME_LEN)
								{
									...
									memcpy(playlist[musicfile_num],file_name,strlen(file_name));
									memcpy(lcdlist[musicfile_num],fn,strlen(fn));
									musicfile_num++;//记录文件个数									
								}
							}//if mp3||wav
						 }//else
					} //for
...
//			printf("chgsch:%d,sch:%d\n",chgsch,sch);
		...
		if(bw!=BUFSIZE||result!=FR_OK)
		{
			VS_Soft_Reset();
			GUI_Delay(10);
			if(play_index<musicfile_num-1)		//自动开始下一首歌曲
				play_index++;
			else
				play_index = 0;
...
//			bsp_DelayUS(1000);
		...
		play_index--;
		if(play_index>musicfile_num)play_index=musicfile_num-1;
...
//			bsp_DelayUS(1000);
		...
		play_index++;
		if(play_index>=musicfile_num)play_index=0;
		LISTBOX_SetSel(hItem,play_index);
		//printf("paly_index=%d\n",play_index);
		// USER END
        break;
